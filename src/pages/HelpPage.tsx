...
export const HelpPage: React.FC = () => {
  ...
  const faqs = [
    ...
    {
      q: t('help.faqLockQ', 'What does the lock icon on some cloud backups mean?'),
      a: t(
        'help.faqLockA',
        'Files with a lock icon (.enc) are encrypted using client-side AES-256-GCM. They cannot be read by anyone (including Google) without your encryption password. You will be prompted for your password to unlock and decrypt them.'
      ),
    },
    {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <div className="text-center space-y-3 max-w-xl mx-auto">
        <div className="flex items-center justify-center gap-2 flex-wrap">
          ...
          <span className="inline-flex items-center space-x-1 px-3 py-1 rounded-full bg-slate-100 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08] text-slate-700 dark:text-slate-300 text-xs font-mono font-medium">
            <span>v{APP_VERSION}</span>
            <span className="text-slate-400 dark:text-slate-500">•</span>
            <span className="text-[11px] text-slate-500">{APP_BUILD_DATE}</span>
          </span>
        </div>
